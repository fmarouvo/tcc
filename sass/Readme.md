...
folder are mapped to the application's root namespace, 'FacilDesktop'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "FacilDesktop"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'FacilDesktop' in the same way
as `"FacilDesktop/sass/src"`.
